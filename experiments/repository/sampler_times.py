...
class SamplerTimesSolution(EnvExperiment):
    ...
    def run(self):
        ...
        # ----------------------------------------------------------------------      
        # Delaying local time pointer so it is aligned with the middle point1
    ...
    def analyze(self):
        dts = [self.core.mu_to_seconds(self.timestamps[i+1] - self.timestamps[i]) for i in range(2*N_SAMPLES)]
        dts_us = [dt * 1e6 for dt in dts]
        expected_timeline = [(PERIOD_US / 4 + i * PERIOD_US / 2)*1e6 for i in range(2*N_SAMPLES)]
